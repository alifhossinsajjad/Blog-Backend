...
export const imageWorker = new Worker(IMAGE_PROCESSING_QUEUE, async (job) => {
  ...
  try {
    ...
    console.log(`[Job ${job.id}] Successfully processed and uploaded image for post ${postId}`);
    return thumbnailUrl;
  } catch (error) {
    console.error(`[Job ${job.id}] Error processing image for post ${postId}:`, error);
